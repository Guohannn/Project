...
public class Disk_Write extends Thread{
	...
	public static boolean ifDisk_Write=false;//是否发生磁盘写操作
	...
	public static void Disk_WritePrint(Memory memory,PCB pcb)
	{
		...
		System.out.println(ClockInterrupt.time+"："+"[出缓冲区："+pcb.ProID+"："+pcb.Instruc[pcb.ProID].Instruc_ID+"，类型5，磁盘文件写操作函数："+pcb.Instruc[pcb.ProID].L_Address+" "+physicaladdress+" "+i+" "+disk_physicaladdress+"]");
		GUI.ProRec.append(ClockInterrupt.time+"："+"[出缓冲区："+pcb.ProID+"："+pcb.Instruc[pcb.ProID].Instruc_ID+"，类型5，磁盘文件写操作函数："+pcb.Instruc[pcb.ProID].L_Address+" "+physicaladdress+" "+i+" "+disk_physicaladdress+"]\n");
		ClockInterrupt.Scheduling_info+=(ClockInterrupt.time+"："+"[出缓冲区："+pcb.ProID+"："+pcb.Instruc[pcb.ProID].Instruc_ID+"，类型5，磁盘文件写操作函数："+pcb.Instruc[pcb.ProID].L_Address+" "+physicaladdress+" "+i+" "+disk_physicaladdress+"]\n");
